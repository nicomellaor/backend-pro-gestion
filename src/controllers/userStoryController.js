...
const postUserStory = async (req, res, next) => {
    try {
        const { projectId } = req.params;
        const { storyId, story, priority, estimationPoints, criteria } = req.body;
        ...
        const exists = await UserStory.findOne({ storyId });
        if (exists) {
            return res.status(400).json({ msg: 'UserStory ya está registrado' });
        }

        const userstory = new UserStory(projectId, storyId, story, priority, estimationPoints, criteria);
